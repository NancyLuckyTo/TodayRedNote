...
- 后端
  ...
  - 部署：Railway
- Monorepo 与共享类型
  - 项目采用 Monorepo 架构管理。通过 Workspace 机制在 packages/types 目录中维护前后端共用的 TypeScript 接口（如IPost、IDraft等），保证前后端数据结构的对齐度。
...
- 前置条件
  ...
  - 本地可访问 MongoDB（可使用 MongoDB Atlas 或本地实例）
- 安装依赖
  - 在项目根目录执行：pnpm install
- 配置后端环境变量（server/.env）
  - 在 server 目录下创建 .env 文件，内容示例：

```env
# MongoDB
MONGODB_URI="mongodb+srv://<username>:<password>@<cluster>/<db-name>?retryWrites=true&w=majority"
PORT=3000
...
ALI_OSS_BUCKET="<your-oss-bucket-name>"
ALI_OSS_REGION="<your-oss-region>" # 例如：oss-cn-hongkong
...
- 启动服务
  - 推荐在项目根目录一键启动：
    - pnpm dev
    - 会先构建共享类型包，再并行启动 client 和 server 的开发服务器
  - 或分别启动：
    - 后端：在根目录执行 pnpm dev:server（或进入 server 目录执行 pnpm dev）
    - 前端：进入 client 目录执行 pnpm dev
- 访问应用
  - 浏览器打开前端 Dev Server 地址（默认 http://localhost:5173）

2. 生产部署：前端（Vercel）

- 本项目的前端目前部署在 Vercel，形态是纯前端 SPA，构建产物来自 client/dist
- 关键配置在根目录 vercel.json：
  - buildCommand: "pnpm run build"：
    - 会先构建共享类型包，再构建 client 和 server（按 Monorepo 脚本执行）
  - outputDirectory: "client/dist"：告诉 Vercel 前端静态资源的输出目录
  - installCommand: "pnpm install"：使用 pnpm 安装依赖
  - rewrites：
    - "/api/:path*" -> "https://<your-server-domain>/api/:path*"
    - 开发中当前指向 Railway 的后端地址；
    - 根据实际的后端部署域名，只需在 vercel.json 中调整此配置并重新部署即可。

3. 生产部署：后端（Railway）

- 后端是一个基于 Express 的 Node.js 服务，入口为 server/src/bin/www.ts，构建后运行 dist/bin/www.js。
- 在 Railway 上的典型配置可以是：
  - 安装命令：pnpm install
  - 构建命令：pnpm run build:server
    - 等价于先构建共享类型包再构建 server。
  - 启动命令：pnpm --filter server run start
- 在 Railway 的环境变量面板中配置与本地 .env 相同的一组变量：
  - MONGODB_URI：指向生产库。
  - PORT：Railway 一般会自动注入端口，确保代码中使用该变量。
  - JWT_SECRET：生产环境的更长、更复杂的密钥。
  - OSS 相关变量和硅基流动相关变量同上。
- 部署完成后，会得到一个后端基础域名，例如：https://your-app.up.railway.app：
  - 前端生产环境的 VITE_API_BASE_URL 应指向 https://your-app.up.railway.app/api；
  - 同时在 vercel.json 中将 /api 重写到这个后端地址，实现前后端联调。
...
- 首页是两列瀑布流布局，整体视觉、互动体验贴近小红书与抖音等主流应用的：
  - 每个笔记卡片上半部分是图片封面，下半部分是 1~2 行正文预览与作者信息。
...
- WaterfallContainer 只负责：
  ...
  - 按谁矮放谁的策略，动态地把笔记卡片分配到左列或右列。
- 实现上是一个很朴素的贪心策略：
  - 一开始两列高度都是 0；
  - 每放入一个笔记卡片，就放到当前高度更矮的那一列，并更新那一列的总高度。
    3.3 布局细节：flex + min-w-0
...
- 容器组件：client/src/components/WaterfallContainer.tsx
  - 接收 items、scrollTop、containerHeight、overscan 等参数，把每一列视为一条很长的有序列表，列表中每一项记录自己的 top 和 height。
  - 根据列表数据和已测量高度，先用贪心算法把卡片分配到左右两列。
- 视口裁剪：
  - 由外层页面（HomePage / ProfilePage）传入 scrollTop 和 containerHeight，再结合 overscan 计算出「可见区 + 缓冲区」的上下边界。
  - 对左右两列分别用二分查找，快速找到第一个进入视口的卡片和第一个离开视口的卡片，只渲染这段子数组，其余区域通过 paddingTop / paddingBottom 占位，保证滚动条高度与真实列表一致。
- 实际高度回填：
  - 每个渲染出来的卡片都包在 MeasuredItem 中，内部用 ResizeObserver 监听真实高度变化，通过 onHeightChange 写回 measuredHeights。
  - 下一次布局计算时优先使用真实高度替换预估高度，让瀑布流在不打断滚动的前提下逐步纠正卡片位置。
- 性能价值：
  - 首页和个人主页一次只挂载视口附近的少量 PostCard，即使信息流拉到几百条，DOM 数量和重排成本依旧可控。
  - 配合骨架屏与首屏图片的优先加载，可以在移动端长时间滚动时保持接近原生 App 的流畅度。
...
- 在 postUtils.ts 中定义了封面图的比例元数据：
  - landscape：4:3
  - portrait：3:4
- 每个笔记有一个 coverRatio 字段，默认 portrait（竖图，更贴近主流内容社区）
- 在卡片渲染时：
...
- 首页里传入的 onRefresh 做了两件事：
  - 清空本地记录的“已浏览笔记 ID”（让推荐逻辑重新生成一个“全新流”）。
  - 调用 fetchPosts() 重拉首页数据。
...
- 行为是这样被用起来的：
  ...
  - 在返回详情给前端后，异步地调用 trackUserBehavior(userId， postId， 'view')。
  - 这样浏览行为不会拖慢接口响应。
...
- 个性化推荐核心：postService.getPersonalizedFeed（services/postService.ts）
  - 读取用户画像，取出兴趣最高的若干个标签。
  - 用 tags: { $in: interestTagIds } 作为查询条件，从笔记表里找出匹配这些标签的笔记。
  - 结果按创建时间倒序排序：
...
- 在 HTML 里有一个脚本，会提前请求首页 /api/posts，把结果挂在 window.**PREFETCH_POSTS** 上；
...
4. 配置 CDN 加速（Cloudflare + OSS 静态资源域名）

- 静态资源域名规划
  - 为图片单独规划 img.todayrednote.top 子域，CNAME 指向阿里云 OSS，并通过 Cloudflare 代理作为 CDN 边缘节点，对图片流量进行专门加速。
- 与上传链路深度打通
  - 结合上传流程中设置的 Cache-Control: public, max-age=31536000，让图片天然适配 CDN 缓存策略，形成“浏览器缓存 + Cloudflare 边缘缓存 + OSS 源站”的分层缓存体系，减少回源请求。
- 主站与图片域名解耦
  - 主站流量（todayrednote.top / www.todayrednote.top）仍由 Vercel 直接承载，图片走 img 域名 + CDN，避免不必要的头部开销，降低 OSS 带宽与回源压力，首页图片加载更稳定、更顺滑。

5. 发布进度横幅：即时反馈 + 后台上传
   5.1 体验目标

- 点击“发布”后立即跳转首页，无需等待上传完成；
- 首页顶部悬浮横幅实时展示发布进度，让用户随时知道“笔记到哪了”；
- 发布成功后点击横幅可直接跳转到新笔记详情页；
- 3 秒后横幅自动消失，不打扰浏览。
  5.2 技术实现
- 状态管理：client/src/stores/publishingStore.ts
  - 用 Zustand 存储发布状态（uploading / success / error）、进度百分比、封面图 URL、新笔记 ID；
  - 提供 startPublishing、updateProgress、updateCoverImage、setSuccess 等 action。
- 发布流程：client/src/hooks/useCreatePost.ts
  - 图片上传完成后，用真实 CDN URL 替换预览 blob URL（避免页面刷新后图片丢失）；
  - 分阶段更新进度：10% → 70%（图片上传）→ 90%（API 提交）→ 100%（完成）；
  - 发布成功后将 postId 写入 Store，横幅即可跳转。
- 横幅组件：client/src/components/PublishingBanner.tsx
  - 固定定位在首页顶部，z-index 最高层，不影响下方内容滚动；
  - 左侧展示封面缩略图，中间显示“正在上传中...” / “发布成功！”；
  - 底部红色进度条动态更新；
  - useEffect 监听 status === 'success'，3 秒后自动 reset()。
- 首页集成：client/src/pages/HomePage.tsx
  - 在顶部导航栏下方直接渲染 <PublishingBanner />；
  - 当 status === 'idle' 时组件返回 null，不占任何空间。
    5.3 实用价值
    传统方案
    本方案
    发布时全屏 loading，用户只能干等
    立即回到信息流，继续刷内容
    失败时可能无感知
    横幅明确提示失败，可重试
    发完后手动找新笔记
...
- Tiptap 富文本编辑 + 草稿本地/云端双保险，让创作体验尽量无“心理负担”——随时写，随时走，不怕丢；
- Monorepo + 共享类型（前后端统一数据模型）+ 前后端分离部署（Vercel + Railway），配合规范的鉴权、存储与缓存策略，为长期维护和多人协作提供了清晰的边界和扩展空间；
